  ...
  <div class="app-container">
        ...
        <el-col :span="24">
          <el-form-item label="身份证正面">
            <span v-for="item in borrower.borrowerAttachVOList" :key="item.id">
              <el-image
                v-if="item.imageType == 'card1'"
                style="width: 150px"
                :src="item.imageUrl"
                :preview-src-list="[item.imageUrl]"
              ></el-image>
              <!-- preview-src-list预览图片列表 -->
            </span>
          </el-form-item>
        </el-col>
      </el-row>
    ...
    <el-form label-width="170px" v-if="borrower.status === '认证中'">
      ...
      <el-form-item v-if="approvalForm.status == 2" label="身份证信息是否正确">
        ...
        <span style="color: indianred">（可获取30积分）</span>
      </el-form-item>
      ...
      <el-form-item v-if="approvalForm.status == 2" label="车辆信息是否正确">
        ...
        <span style="color: indianred">（可获取60积分）</span>
      </el-form-item>
      ...
      <el-form-item v-if="approvalForm.status == 2" label="房产信息是否正确">
        ...
        <span style="color: indianred">（可获取100积分）</span>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    back() {
      this.$router.push({ path: '/core/borrower/list' }) // 路由跳转
    },
    ...
    approvalSubmit() {
      ...
      borrowerApi.approval(this.approvalForm).then((response) => {
        ...
        this.$router.push('/core/borrower/list') // 页面跳转
      })
    },
  },
}
