...
export default {
  ...
  methods: {
    ...
    showInfo() {
      ...
      // 解析JSON字符串，转为json字符串传输的数据会将空格、双引号等的特殊字符转码，所以拿到的json数据要解析才能使用
      userInfo = JSON.parse(userInfo)

      // 请求头中携带token，并在后端校验token是否合法
      this.$axios({
        url: '/api/core/userInfo/checkToken',
        method: 'get',
        // headers: {    客户端访问许多页面都要携带token，因此，在axios.js的请求拦截器中将token放到请求头
    ...
    //退出
    logout() {
      cookie.set('userInfo', '') // 退出之前销毁cookie
